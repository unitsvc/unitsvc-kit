 ...
 */
public abstract class AbstractObjectDiffV2 {
     ...
     * @param sourceObject 原始对象
     * @param targetObject 目标对象
     * @param diffConfigs  比对配置
     * @return 字符串
     * @throws Exception 异常
     */
    public static String genChineseDiffStr(Object sourceObject, Object targetObject, List<DiffConfig> diffConfigs) throws Exception {
        List<DiffWrapper> diffWrappers = generateDiff(sourceObject, targetObject, diffConfigs);
     ...
     * @throws Exception 异常
     */
    public static List<DiffWrapper> generateDiff(Object sourceObject, Object targetObject, List<DiffConfig> diffConfigs) throws Exception {
        String typeName = "";
        ...
        // 处理比对配置类
        Map<String, Map<String, DiffConfig.FieldConfig>> configMap = null;
        if (CollectionUtils.isNotEmpty(diffConfigs)) {
            configMap = new HashMap<>(diffConfigs.size());
            for (DiffConfig config : diffConfigs) {
                List<DiffConfig.FieldConfig> fieldConfigs = config.getFieldConfigs();
                if (CollectionUtils.isNotEmpty(fieldConfigs)) {
                    Map<String, DiffConfig.FieldConfig> fieldConfigMap = new HashMap<>(fieldConfigs.size());
                    for (DiffConfig.FieldConfig fieldConfig : fieldConfigs) {
                        fieldConfigMap.put(fieldConfig.getProperty(), fieldConfig);
                    }
                    configMap.put(config.getSimpleClassName(), fieldConfigMap);
                }
            }
        }
        return generateDiff("", "", typeName, sourceObject, targetObject, configMap);
    }
     ...
     * @param sourceObject 原始对象
     * @param targetObject 目标对象
     * @param configMap    比对配置
     * @return
     * @throws Exception 异常
     */
    private static List<DiffWrapper> generateDiff(String path, String cnName, String typeName, Object sourceObject, Object targetObject, Map<String, Map<String, DiffConfig.FieldConfig>> configMap) throws Exception {
        // 过滤都为空的数据
        ...
        // 获取对象定义
        Map<String, DiffConfig.FieldConfig> classConfigMap = Optional.ofNullable(configMap).map(v -> v.get(simpleName)).orElse(null);
        for (final Field field : fields) {
            ...
            if (null != configMap) {
                // 配置与注解二选一，不存在混合使用
                if (null != classConfigMap) {
                    // ------------------------------- 基于配置 --------------------------------------------
                    DiffConfig.FieldConfig fieldConfig = classConfigMap.get(field.getName());
                    if (null != fieldConfig) {
                        if (cnName == null || "".equals(cnName)) {
                            // 根节点直接取注解名称
                            nameCn = fieldConfig.getNameCn();
                        } else {
                            // 子节点进行字段拼接
                            nameCn = cnName + "." + fieldConfig.getNameCn();
                        }
                        if (Boolean.TRUE.equals(fieldConfig.getIgnore())) {
                            // 忽略该比对字段
                            continue;
                        }
                    } else {
            ...
            if (Collection.class.isAssignableFrom(type)) {
                ...
                // ---------------------------------------- 基于配置 ---------------------------------------------
                // 获取泛型类名称
                String genericSimpleName = genericClass.getSimpleName();
                // 获取对象定义
                Map<String, DiffConfig.FieldConfig> genericClassConfigMap = Optional.ofNullable(configMap).map(v -> v.get(genericSimpleName)).orElse(null);
                for (Field collectionField : collectionFields) {
                    if (null != configMap) {
                        // 配置与注解二选一，不存在混合使用
                        if (null != genericClassConfigMap) {
                            DiffConfig.FieldConfig collFieldConfig = genericClassConfigMap.get(collectionField.getName());
                            if (null != collFieldConfig && Boolean.TRUE.equals(collFieldConfig.getIsPrimary())) {
                                keyField = collectionField;
                                // 集合主键字段名称，只取第一个
                                keyNameCn = collFieldConfig.getPrimaryName();
                                break;
                            }
                        }
                    } else {
                ...
                for (Object result : resultSet) {
                    ...
                    // 递归处理集合字段差异
                    List<DiffWrapper> collectDiff = generateDiff(opPath, opNameCn, genericClass.getTypeName(), oldObj, newObj, configMap);
                    if (collectDiff != null) {
                ...
                // 递归出口
                // 比对单个字段差异
                DiffWrapper diffWrapper = generateOneDiffs(newPath, nameCn, field, sourceObject, targetObject, null != configMap, classConfigMap);
                if (diffWrapper != null) {
                    diffWrappers.add(diffWrapper);
                }
            }
     ...
     * @param source         原始对象
     * @param target         目标对象
     * @param enableConfig   启用配置
     * @param fieldConfigMap 字段配置
     * @return 差异
     * @throws Exception 异常
     */
    private static DiffWrapper generateOneDiffs(String path, String nameCn, Field field, Object source, Object target, boolean enableConfig, Map<String, DiffConfig.FieldConfig> fieldConfigMap) throws Exception {
        ...
        String dateFormat = "";
        if (enableConfig) {
            if (null != fieldConfigMap) {
                DiffConfig.FieldConfig fieldConfig = fieldConfigMap.get(field.getName());
                if (fieldConfig != null) {
                    dateFormat = fieldConfig.getDataFormat();
                    // 过滤忽略的字段key
                    if (Boolean.TRUE.equals(fieldConfig.getIgnore())) {
                        return null;
                    }
