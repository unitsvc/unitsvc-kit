...
import com.unitsvc.kit.facade.db.antlr.d4.inner.ExprBaseVisitor;
import com.unitsvc.kit.facade.db.antlr.d4.inner.ExprParser;
...
public class FilterVisitor extends ExprBaseVisitor<Document> {
    ...
    public Document visitArrayCondition(ExprParser.ArrayConditionContext ctx) {
        ...
        for (ExprParser.ValueContext value : values) {
            ...
            if (null != b1) {
                ...
                // 去除转义字符
                params.add(StrUtil.replaceStr(s1.getText()));
            } else if (null != n2) {
                params.add(null);
            } else if (null != idStr) {
                params.add(idStr.getText());
            }
        }
    ...
    public Document visitValueCondition(ExprParser.ValueConditionContext ctx) {
        ...
        TerminalNode idStr = ctx.value().ID();
        ...
        if (null != b1) {
            ...
            // 去除转义字符
            value = StrUtil.replaceStr(s1.getText());
        } else if (null != n2) {
            value = null;
        } else if (null != idStr) {
            value = idStr.getText();
